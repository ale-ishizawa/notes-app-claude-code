...
export default function NoteDetailPage() {
  ...
  if (!note) return <div className="p-6 text-muted-foreground text-sm">Loading...</div>
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm text-muted-foreground">v{note.version}</span>
        <div className="flex-1" />
        {canEdit && (
          <>
            <Button size="sm" variant="ghost" onClick={() => setShowShare(!showShare)}>
              <Share2 className="h-4 w-4 mr-2" />Share
            </Button>
            <Button size="sm" variant="ghost" onClick={() => setShowVersions(!showVersions)}>
              <History className="h-4 w-4 mr-2" />History
            </Button>
            <Button size="sm" variant="ghost" onClick={handleSummarize} disabled={summarizing}>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex flex-wrap gap-1 items-center">
          {tags.map(t => (
            <span key={t} className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-accent text-accent-foreground text-xs">
          ...
          {canEdit && (
            <div className="flex gap-1">
              ...
              />
              <Button size="sm" variant="ghost" className="h-6 px-2 text-xs" onClick={addTag}>+</Button>
            </div>
          )}
        </div>
      </div>
      ...
      {showShare && canEdit && (
        <div className="border border-border rounded-lg p-4 space-y-3">
          <h3 className="font-medium text-sm">Share with team member</h3>
        ...
        <div className="border border-border rounded-lg p-4 space-y-3 bg-accent/30">
          <div className="flex items-center justify-between">
            <h3 className="font-medium text-sm flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">({aiSummary.status})</span>
            </h3>
            {aiSummary.status === 'completed' && (
              <div className="flex gap-2">
                <Button size="sm" variant="outline" onClick={() => handleSummaryAction('accept')}>Accept</Button>
                <Button size="sm" variant="ghost" onClick={() => handleSummaryAction('reject')}>Reject</Button>
              </div>
            )}
          </div>
          {aiSummary.summary?.summary && <p className="text-sm">{aiSummary.summary.summary}</p>}
      ...
      {showVersions && (
        <div className="border border-border rounded-lg p-4 space-y-3">
          <h3 className="font-medium text-sm">Version History ({versions.length} versions)</h3>
          {versions.length === 0 ? (
              ...
              {versions.map((v) => (
                <div key={v.id} className="flex items-center justify-between text-sm py-2 border-b border-border last:border-0">
                  <div>
                    ...
                    <p className="text-muted-foreground truncate max-w-xs">{v.title}</p>
                  </div>
                  <div className="flex gap-2">
                    <Button size="sm" variant="ghost" onClick={() => setDiffBase(diffBase?.id === v.id ? null : v)}>
          ...
          {diffBase && (
            <div className="space-y-2">
              ...
              <p className="text-xs font-medium">Title diff:</p>
              <DiffViewer oldText={diffBase.title} newText={note.title} />
              <p className="text-xs font-medium mt-2">Content diff:</p>
